...
int main (void) {
	...
	xAxis.set_color(Graph_lib::Color::blue);
	xAxis.label.set_color(Graph_lib::Color::blue);
	win.attach (xAxis);

	Graph_lib::Axis yAxis (Graph_lib::Axis::y, Graph_lib::Point(20, 300), 280, 10, "Y");
	yAxis.set_color(Graph_lib::Color::blue);
	yAxis.label.set_color (Graph_lib::Color::blue);
	win.attach (yAxis);

	Graph_lib::Function sine (std::sin, 0, 100, Graph_lib::Point(20, 150), 1000, 50, 50);
	sine.set_color (Graph_lib::Color::blue);
	sine.set_style (Graph_lib::Line_style (Graph_lib::Line_style::solid, 5));
	win.attach (sine);
	...
	poly_rect.add (Graph_lib::Point (60, 60));
	...
	Graph_lib::Image pic (Graph_lib::Point(200, 150), "image.jpg");
	win.attach(pic);
...
//									image is constructed above with to suit this after move
	...
	sizes.set_font (Graph_lib::Font::times_bold);
	sizes.set_font_size (40);
	win.attach (sizes);

	Graph_lib::Image pic2 (Graph_lib::Point(225, 225), "snow_cpp.gif");
	pic2.set_mask (Graph_lib::Point(40,40), 200, 150); // crops the image
	win.attach(pic2);
