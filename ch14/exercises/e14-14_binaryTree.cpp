...
namespace Graph_lib {
	...
	class Binary_tree : public Shape {
		...
		protected :
			...
			vector<std::string> getLabels () const {
				return labels;
			}
				...
				for (int i = 0; i < levels; ++i, h_pos /= 2, nodeCnt *= 2, v_pos += screen_h/levels) {
					for (int j = 0, pos = h_pos/2; j < nodeCnt; ++j, pos += h_pos) {
						...
						if (i > 0) {
							...
							if (i > 1)
								parentLabel = labels[pow (2,i-1) + j/2 - 1];

							if (j % 2 == 0)
								labels.push_back (parentLabel + "l");
							else
								labels.push_back (parentLabel + "r");
						} else
	...
	class Binary_tree_triangle : public Binary_tree {
		public :
			...
			void draw_lines () const override {
				...
				vector<std::string> labels = getLabels();
				for (int i = 0; i < nodes.size(); ++i) {
					...
					if (i > 0)  {
						...
						if (i % 2)  {
							Arrow (parent.bottomLeft(), current.top(), Binary_tree::getLineType()).draw();
						} else {
							Arrow (parent.bottomRight(), current.top(),	Binary_tree::getLineType()).draw();
						}
					}
					Text (nodes[i], labels[i]).draw();
				}
