...
const char roman	= 'r';	// token for roman numeral
...
//		double value;
		...
		Token (char ch, Roman_int val) : kind (ch), name(""), value (val) {
		}
...
//		double value;
	...
	Variable (std::string n, Roman_int v, bool c) : name (n), value (v), constant (c) {
	}
};
...
class Symbol_table {
	...
	public:
		Roman_int	get	(std::string s);
		void		set	(std::string s, Roman_int d);
		bool		declared	(std::string s);
		Roman_int	declare	(std::string var, Roman_int val, bool constant);
};	

Roman_int Symbol_table::get (std::string s) { // return the value of the Variable named s
...
void Symbol_table::set (std::string s, Roman_int d) { // set the variable named s to d
	for (int i = 0;  i < var_table.size(); ++i)
		if (var_table[i].name == s) {
...
Roman_int Symbol_table::declare (std::string var, Roman_int val, bool constant) {
	if (declared (var))
...
Roman_int	declaration (Token_stream& ts, bool constant);
Roman_int	expression (Token_stream& ts);	// read and evaluate expression
Roman_int	term (Token_stream& ts);
Roman_int	factorial (Token_stream& ts);
...
Roman_int statement (Token_stream& ts) {
	...
	switch (t.kind) {
		case let :	{
			Roman_int d = declaration(ts, false);
			return d;
		}
		case con : {
			Roman_int d = declaration(ts, true);
			return d;
		}
		default	: 
			ts.putback(t);
...
Roman_int expression (Token_stream& ts) {
	Roman_int left = term (ts);	// read and evaluate an Exrpession
	while (true) {
...
Roman_int term (Token_stream& ts) {
	...
	while (true) {
		...
		//		left = left - d * ((int) (left/d));		// my solution
				left %= d;		// for the purpose of this exercise
...
*/
			...
			case '*':
				...
				break;
			case '/': {
				Roman_int divisor = factorial(ts);
				if (divisor == 0)
					throw std::runtime_error ("Cannot divide by zero");
			...
			case '^': {
				Roman_int right = factorial(ts);
				Roman_int result (0);

				for (; right.as_int() > 0; --right, result *= left);
...
Roman_int factorial (Token_stream& ts) {
	...
	if (t.kind == '!') {
		Roman_int factorial = 1;
		for (Roman_int i = left; i > 0; --i)
			factorial *= i;
...
/*		case '-':
			d = primary(ts); 
			return d > 0 ? -d : 0;	// this is to avoid -0 error
	/ *	case number :	// we use '8' to represent a number
		...
		case root :
			...
			return Roman_int(sqrt (d.as_int()));
		case power : {
			t = ts.get ();
			if (t.kind != '(')
				throw std::runtime_error ("'(' expected");
