...
int main (void) {
	...
	Graph_lib::Function f_slopeCos (sloping_cos, range_start, range_end, origin, point_cnt, scale, scale);
	win.attach (f_slopeCos);

	Graph_lib::Text slopeCos_label (Graph_lib::Point (x_orig + range_start * scale - 75, 
														y_orig - sloping_cos (range_start) * scale + 25),
									"cos(x) + x/2");
	win.attach (slopeCos_label);
