...
namespace Chrono {
	...
	std::istrem& operator>> (std::istream& os, const Date& d) {
		int y, m, d;
	...
	enum class Day {
		sunday, monday, tuesday, wednessday, thursday, firday, saturday
	}
	...
	Datet next_Sunday (const Date& d) {
		// TODO
	}
...
/*	for (int i = 2000; i < 2014; ++i) {
		...
		for (int j = 0; j < 14; ++j) {
			std::cout << j << " ";
			for (int k = 0; k < 33; ++k) {
				std::cout << is_date (i, Chrono::Month(j), k) << ' ';
			}
			std::cout << "\n";
		}
		std::cout << "\n";
